    ...
    <div class="listings-page" v-if="listings">
        <div class="listingtext" style="width: 100%; display: flex;">
            <button class="block py-1 px-2 rounded md:bg-light-orange md:text-white text-lg font-default-font"
                @click="sort('propertyName')" style="width: 175px;">Property Name</button>
            <button class="block py-1 px-2 rounded md:bg-light-orange md:text-white text-lg font-default-font"
                @click="sort('rating')">Rating</button>
            <button class="block py-1 px-2 rounded md:bg-light-orange md:text-white text-lg font-default-font"
                @click="sort('rent')">Price</button>
            ...
            <table style="margin-left: 0; width: 50%; border: 1px solid black; box-shadow: 6px 0 5px -2px #888;">
                <tbody>
                    <tr v-for="listing in listings" :key="listing.propertyName">
                        <td>
                            <div v-bind:id="listing.propertyName" class="propertydiv"
                                @mouseover="propertyHover = true, getPropertyData($event.target.id)"
                                @mouseleave="propertyHover = false">
        ...
        <div v-show="propertyHover" class="propertydiv" style="float: bottom;">
            Amenities: <span v-for="(amenity, index) in this.property.amenities">
                {{ amenity }}{{ (index+1 < this.property.amenities.length) ? ', ' : '' }} </span> <br>
